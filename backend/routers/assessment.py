...
@router.get("/state")
def get_assessment_state(db: Session = Depends(get_db), user: CurrentUser = Depends(get_current_user)):
...
async def submit_final(db: Session = Depends(get_db), user: CurrentUser = Depends(get_current_user)):
    ...
    if missing:
        raise HTTPException(
            status_code=400,
            detail={
                "message": "Every question must be answered before final submission.",
                "missing": [{"questionId": q.id, "caseId": q.case_id, "question": q.question} for q in missing],
            },
        )
    ...
    await log_activity(
        ...
        detail=f"{user.full_name} submitted the final assessment — {final_score}/100 ({rating})", db=db,
    )
